...
const btn = (label: string, type: 'digit'|'op'|'fn'|'eq'|'clear'|'mem') => ({ label, type })
const BASIC_BTNS = [
...
export default function CalculatorApp() {
  ...
  const [parens, setParens]   = useState(0) // open paren count

  const pushHistory = (e: string, r: string) =>
    setHistory(h => [{ expr: e, result: r }, ...h].slice(0, 12))

  const handleInput = useCallback((label: string) => {
    setDisplay(prev => {
      if (label === 'C') { setExpr(''); setParens(0); return '0' }
      if (label === '⌫') {
      ...
      if (label === 'π') { setExpr(e => e + prev + 'π'); return 'π' }
      if (label === 'e') { setExpr(e => e + prev + 'e'); return 'e' }
      if (label === 'x²') { setExpr(e => e + prev + '^2'); return prev + '²' }
      if (label === 'xʸ') { setExpr(e => e + prev + '^'); return prev + '^' }
      if (label === '(') { setParens(p => p + 1); setExpr(e => e + prev + '('); return '(' }
      if (label === ')') {
        if (parens === 0) return prev
        setParens(p => p - 1); setExpr(e => e + prev + ')'); return ')'
      }
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      else if (k === '/') { e.preventDefault(); handleInput('÷') }
  ...
  const renderGrid = (rows: typeof BASIC_BTNS) => rows.map((row, ri) => (
    <div key={ri} style={{ display: 'flex', gap: 6 }}>
      {row.map(({ label, type }) => (
        <button
          ...
          style={{
            flex: 1, height: 48, borderRadius: 10, fontSize: type === 'fn' ? 11 : 16,
            fontWeight: 600, cursor: 'pointer', border: '1px solid rgba(255,255,255,0.07)',
  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: 'rgba(8,12,20,0.6)' }}>
      {/* Calculator */}
      <div style={{ display: 'flex', flexDirection: 'column', width: mode === 'scientific' ? 320 : 280, padding: 16, gap: 6, flexShrink: 0 }}>
        {/* Mode toggle */}
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 4 }}>
          <button onClick={() => setMode(m => m === 'basic' ? 'scientific' : 'basic')}
        ...
        {/* Expression */}
        <div style={{ textAlign: 'right', fontSize: 11, color: 'rgba(255,255,255,0.35)', fontFamily: 'monospace', minHeight: 16, wordBreak: 'break-all' }}>
          {expr || ' '}
        </div>

        {/* Display */}
        <div style={{
          textAlign: 'right', fontSize: display.length > 12 ? 20 : display.length > 8 ? 28 : 38,
          fontWeight: 700, color: isErr ? '#f87171' : '#f0f4f8',
          fontFamily: '"JetBrains Mono", monospace', letterSpacing: '-0.02em',
          padding: '6px 4px', wordBreak: 'break-all', lineHeight: 1.1,
        }}>{display}</div>
      ...
      {/* History */}
      <div style={{ flex: 1, borderLeft: '1px solid rgba(255,255,255,0.06)', padding: '14px 12px', overflowY: 'auto' }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.25)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>History</div>
        {history.length === 0 ? (
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)', textAlign: 'center', marginTop: 40 }}>No calculations yet</div>
        ) : history.map((h, i) => (
          ...
          >
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', fontFamily: 'monospace', marginBottom: 2 }}>{h.expr}</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--cryo-accent)', fontFamily: 'monospace' }}>{h.result}</div>
          </button>
        ))}
      </div>
    </div>
